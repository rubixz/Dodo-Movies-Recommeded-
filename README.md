...
1. Precomputed data — A dataset of movies and a precomputed similarity matrix are loaded
   from pickle files ( model/movie_list.pkl and model/similarity.pkl ).
...
3. Movie selection — The user picks a movie from a dropdown (populated with all movie titles
   in the dataset).
   
5. Recommendation logic — When “Show Recommendation” is clicked, the app:
...
7. Poster retrieval — For each recommended movie, the app calls the TMDB API using the
   movie’s ID to fetch its poster image URL.
   
9. Display — The 5 recommended movies are shown side-by-side, each with its title and poster.
...
│ └── similarity_topk.pkl # Precomputed similarity matrix
